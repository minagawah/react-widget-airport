...
export const PlaneGraphics = CustomPIXIComponent(
  {
    ...
    customApplyProps: function (g, oldProps, newProps) {
      ...
      if (path.length > 1) {
        g.lineStyle(1, plane_path_color, 0.2);
        ...
        //   const spacing = PLANE_PATH_SPACING;
        //   let angle = mathlib.angle(pth1, pth0);
        //   let dx = pth0.x - pth1.x;
        //   let dy = pth0.y - pth1.y;
        //   let dist = Math.sqrt(dx * dx + dy * dy);
        //   _x = pth0.x + Math.cos(angle) * (dist * ((tick % spacing) / spacing));
        //   _y = pth0.y + Math.sin(angle) * (dist * ((tick % spacing) / spacing));
        // } else {
        //   _x = pth0.x;
        //   _y = pth0.y;
        // }
...
export const usePlanes = () => {
  ...
  const resetPlanes = ({ ports = [], num_of_planes }) => {
    ...
    if (ports.length) {
      console.log('[Airport/planes] ++++ resetPlanes()');
      ...
      // console.log('[Airport/plane] planes:');
      for (let i = 0; i < num_of_planes; i++) {
        const p = generate(i);
        // if (i < 10) {
        //   console.log(`[Airport/plane] plane[${i}] (${int(p.x)}, ${int(p.y)})`);
        // }
        _planes.push(p);
      }
